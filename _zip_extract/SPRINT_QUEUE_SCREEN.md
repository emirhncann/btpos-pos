# Sprint — Belge Aktarım Ekranı
**Dosya:** `src/screens/DocumentQueueScreen.tsx` (yeni)
...
export function DocumentQueueScreen({ companyId }: { companyId: string }) {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 800, margin: '0 auto' }}>
      ...
      {/* Hatalıları toplu tekrar gönder */}
      {counts.failed > 0 && (
            ...
            return (
              <div key={op.id} style={{ background: 'white', border: '1px solid #E5E7EB',
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  {op.status === 'failed' && (
                    <button onClick={() => void handleRetry(op.id)} disabled={retrying === op.id}
                  ...
                  {op.status === 'success' && (
                    <button onClick={() => void handleDelete(op.id)}
...
## DashboardScreen'e ekle

```tsx
// DashboardScreen'de raporlar/menü bölümüne:
import { DocumentQueueScreen } from './DocumentQueueScreen'

// Sekme olarak:
{ id: 'queue', label: '📤 Belge Aktarım', component: <DocumentQueueScreen companyId={companyId} /> }
```
