...
function paymentLinesFromPavoData(data: unknown, fallback: PaymentLine[]): PaymentLine[] {
  const raw = (data ?? {}) as Record<string, unknown>
  ...
  const realLines: PaymentLine[] = (added as PavoAddedPayment[])
    ...
    .map(p => {
      ...
      return {
        ...
        method: (isCash ? 'cash' : 'card') as PaymentMethodKey,
        amount: Number(p.PaymentAmount ?? 0),
        label: isCash ? 'Nakit' : 'Kart',
        mediator,
        brand: p.Brand != null ? Number(p.Brand) : undefined,
      }
    })
    .filter(l => l.amount > 0)

  return realLines.length > 0 ? realLines : fallback
}

function paymentAmountsFromLines(lines: PaymentLine[]) {
  return {
...
export default function POSScreen({
  ...
  const [heldActiveTab, setHeldActiveTab] = useState<'held' | 'pending'>('held')
  ...
  const [pavoPendingSaleRef, setPavoPendingSaleRef] = useState<{
    saleId:     number | null
  ...
  const pavoIncompleteRef = useRef(false)
  const pavoPendingSaleRefLive = useRef(pavoPendingSaleRef)
  pavoPendingSaleRefLive.current = pavoPendingSaleRef
  ...
  useEffect(() => {
    void window.electron.store.get('terminal_id').then(v => {
      setTerminalId((v as string | null) ?? null)
    }).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const r = await fetch(`${API_URL}/payment-provider-brands/enabled/${terminalId}`, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        })
        if (!r.ok) throw new Error('API başarısız')
        const brands = await r.json() as PaymentProviderBrand[]
        const list = Array.isArray(brands) ? brands : []
        setEnabledBrands(list)
        await window.electron.db.saveEnabledBrands(terminalId, list)
      } catch {
        try {
          const cached = await window.electron.db.getEnabledBrands(terminalId)
          setEnabledBrands(cached)
        } catch {
          /* yok say */
        }
      }
  ...
  async function loadPendingSales() {
    if (!pavoSettings) {
      setPendingError('Pavo cihazı bağlı değil.')
      setPendingSales([])
      return
    }
  ...
  const dueTotal = pavoPendingSaleRef
    ? Math.max(0, parseFloat((pavoPendingSaleRef.totalPrice - pavoPendingSaleRef.paidAmount).toFixed(2)))
    : grandTotal
  ...
  function addPaymentLineWithMethod(
    method: PaymentMethodKey,
    brand: PaymentProviderBrand | null,
    forceAmount?: number,
  ) {
    const raw = forceAmount !== undefined
      ? forceAmount
      : parseFloat(pendingAmount.replace(/\./g, '').replace(',', '.')) || 0

    if (raw <= 0 || remaining <= 0) return

    const isTaksit = method === 'other' && brand?.payment_provider_brand_id === 999

    setPaymentLines(prev => [...prev, {
      id:       crypto.randomUUID(),
      method:   isTaksit ? 'card' : method,
      amount:   Math.round(raw * 100) / 100,
      label:    isTaksit ? 'KK Taksit/Puan'
        : method === 'cash' ? 'Nakit'
        : method === 'card' ? 'Kredi Kartı'
        : brand?.payment_provider_brand_nm ?? 'Diğer',
      mediator: isTaksit ? 2 : method === 'cash' ? 1
        : method === 'card' ? 2 : brand?.payment_mediator ?? 14,
      brand: isTaksit ? 999
        : method === 'other' ? brand?.payment_provider_brand_id : undefined,
    }])
    setPendingAmount('')
    setActiveMethod(null)
  ...
  async function completeSale(forcedLines?: PaymentLine[]) {
    ...
    if (currentOrderNo && pavoSettings && pavoIncompleteRef.current) {
      pavoIncompleteRef.current = false
      try {
        const seq = await window.electron.db.nextPavoSequence()
        const pending = await pavoCheckPendingSale(pavoSettings, seq, currentOrderNo)
  ...
  async function completeSaleWithAddPayment(
    lines: PaymentLine[],
    saleRef: {
      saleId:     number | null
      saleNumber: string | null
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const endpoint = isLast ? 'AddPaymentAndFinalizeSale' : 'AddPayment'
        ...
        const addRes = await pavoAddPayment(
          ...
          seq,
          {
            ...
            orderNo:    saleRef.orderNo || undefined,
            saleId:     saleRef.saleNumber ? null : (saleRef.saleId ?? undefined),
          },
          {
            mediator: line.mediator,
            amount:   line.amount,
        ...
        >
          ...
          >
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>
                  {heldActiveTab === 'held' ? 'Bekletilen Belgeler' : 'Askıdaki Ödemeler'}
                </div>
                <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>
                  {heldActiveTab === 'held'
                    ? `${heldDocs.length} belge`
                    : `${pendingSales.length} askıdaki ödeme`}
                </div>
              </div>
              <button
                type="button"
            ...
            <div style={{
              ...
              {[
                { id: 'held'    as const, label: '📋 Bekletilen', count: heldDocs.length },
                { id: 'pending' as const, label: '⏳ Askıdaki',   count: pendingSales.length },
              ].map(tab => (
                <button
                  key={tab.id}
                  type="button"
                  onClick={() => {
                    setHeldActiveTab(tab.id)
                    if (tab.id === 'pending') void loadPendingSales()
                  }}
                  style={{
                    flex:         1,
                    padding:      '10px 0',
                    ...
                    fontSize:     13,
                    fontWeight:   heldActiveTab === tab.id ? 700 : 500,
                    color:        heldActiveTab === tab.id ? '#111827' : '#9CA3AF',
                    borderBottom: heldActiveTab === tab.id ? '2px solid #111827' : '2px solid transparent',
                    marginBottom: -2,
                    cursor:       'pointer',
                    display:      'flex',
                    alignItems:   'center',
                ...
                >
                  ...
                  {tab.count > 0 && (
                    <span style={{
                      fontSize:     10,
                      fontWeight:   700,
                      background:   heldActiveTab === tab.id ? '#111827' : '#E5E7EB',
                      color:        heldActiveTab === tab.id ? 'white' : '#6B7280',
                      borderRadius: 99,
                      padding:      '1px 6px',
                    }}>{tab.count}</span>
                  )}
              ...
              {heldActiveTab === 'pending' && (
                <>
                  {pendingLoading ? (
                    ...
                    <div key={sale.saleNumber || sale.orderNo || sale.saleId} style={{
                      borderBottom: '1px solid #F3F4F6',
                      padding:      '14px 20px',
                    }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between',
                      ...
                      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                        <div style={{ flex: 1, background: '#F0FDF4', borderRadius: 7,
                          padding: '6px 10px', textAlign: 'center' }}>
                          <div style={{ fontSize: 10, color: '#6B7280' }}>Ödenen</div>
                          <div style={{ fontSize: 13, fontWeight: 700, color: '#16A34A' }}>
                            {fmt(sale.paidAmount)}
                          </div>
                        </div>
                        <div style={{ flex: 1, background: '#FEF2F2', borderRadius: 7,
                          padding: '6px 10px', textAlign: 'center' }}>
                          <div style={{ fontSize: 10, color: '#6B7280' }}>Kalan</div>
                          <div style={{ fontSize: 13, fontWeight: 700, color: '#DC2626' }}>
                            {fmt(sale.remainingPaymentAmount)}
                          </div>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          ...
                          onClick={() => {
                            void (async () => {
                              ...
                              try {
                                const seq = await window.electron.db.nextPavoSequence()
                                const res = await pavoAbandonSuspendedSale(pavoSettings, seq, {
                                  saleId:     sale.saleId,
                        ...
                        {sale.remainingPaymentAmount > 0 && (
                          ...
                          >
                            💳 Kalan Ödemeyi Al ({fmt(sale.remainingPaymentAmount)})
                          </button>
                        )}
                ...
                {menuOpen === 'islemler' && [
                  ...
                  <PopupItem key={i} icon={item.icon} label={item.label} disabled={item.disabled} danger={item.danger} accent={MENU_ACCENT.islemler} layout="stack"
                    onClick={() => {
                      ...
                      if (item.label.startsWith('Belge getir')) { openHeldModal(); setMenuOpen(null); return }
          ...
          <div style={{ padding: '5px 8px 8px', borderTop: '1px solid #f0f0f0', flexShrink: 0 }}>
            {!paymentMode && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                <button
                  onClick={() => {
                    const amt = parseFloat(numBuf.replace(',', '.')) || dueTotal
                ...
                {!returnMode && otherVisibleBrands.length > 0 && (
                  <button
                    ...
                    style={{
                      padding: '11px 4px',
                      borderRadius: 7,
                      border: cart.length === 0 ? 'none' : '1.5px solid #E5E7EB',
                      background: cart.length === 0 ? '#f5f5f5' : '#F9FAFB',
                      color: cart.length === 0 ? '#bdbdbd' : '#374151',
                      fontWeight: 700,
                      fontSize: 13,
                      cursor: cart.length === 0 ? 'default' : 'pointer',
                    }}
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 'clamp(14px, 1.8vh, 17px)', fontWeight: 600, color: '#232733' }}>
                    Parçalı Ödeme
                  </div>
              ...
              <button type="button" onClick={() => {
                ...
                style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center',
                  ...
                  color: '#61656D', cursor: 'pointer', fontSize: 13 }}>✕</button>
            </div>
              ...
              <div style={{
                ...
                <div style={{ padding: '2vh 4%', borderBottom: '1px solid #E3E5E9', flexShrink: 0 }}>
                  <div style={{ fontSize: 'clamp(10px, 1.2vh, 11px)', fontWeight: 600,
                    textTransform: 'uppercase' as const, color: '#989BA3', letterSpacing: '0.04em', marginBottom: '0.8vh' }}>
                    Kalan Tutar
                  </div>
                  <div style={{ fontSize: 'clamp(22px, 4vh, 34px)', fontWeight: 600,
                    fontFamily: 'monospace', color: remaining <= 0 ? '#17845A' : '#232733', lineHeight: 1.1 }}>
                  ...
                  <div style={{ height: '0.8vh', maxHeight: 7, background: '#F1F2F4',
                    borderRadius: 99, marginTop: '1.5vh', overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: 99, transition: 'width 0.35s ease',
                      background: remaining <= 0 ? '#17845A' : '#3457D5',
                      width: `${dueTotal > 0 ? Math.min((paidTotal / dueTotal) * 100, 100) : 0}%`,
                    }} />
                  </div>
                </div>
                ...
                <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                  <div style={{ fontSize: 'clamp(10px, 1.2vh, 11px)', fontWeight: 600,
                    textTransform: 'uppercase' as const, color: '#989BA3', letterSpacing: '0.04em',
                    padding: '1.6vh 4% 1vh' }}>
                    Eklenen Ödemeler
                  </div>
                  {paymentLines.length === 0 ? (
                    ...
                    const badgeMap: Record<string, { bg: string; fg: string; label: string }> = {
                      cash:  { bg: '#FDF1DE', fg: '#A8620A', label: 'NK' },
                      card:  { bg: '#ECF0FD', fg: '#3457D5', label: 'KK' },
                      other: { bg: '#F3E9FB', fg: '#7A3AAB', label: line.label.slice(0, 2).toUpperCase() },
                    }
                    ...
                    return (
                      <div key={line.id} style={{ display: 'flex', alignItems: 'center',
                        ...
                        <div style={{ width: 32, height: 24, display: 'flex', alignItems: 'center',
                          ...
                          borderRadius: 7, flexShrink: 0, background: b.bg, color: b.fg }}>{b.label}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 'clamp(11px, 1.4vh, 12.5px)', fontWeight: 600,
                            color: '#232733', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {line.label}
                          </div>
                        ...
                        <span style={{ fontWeight: 600, fontSize: 'clamp(12px, 1.5vh, 13.5px)',
                          color: '#232733', fontFamily: 'monospace' }}>
                          ₺{line.amount.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}
                        </span>
                        <button type="button" onClick={() => setPaymentLines(prev => prev.filter(l => l.id !== line.id))}
                          style={{ width: 22, height: 22, display: 'flex', alignItems: 'center',
                            justifyContent: 'center', color: '#989BA3', cursor: 'pointer',
                            border: 'none', background: 'transparent', fontSize: 12, borderRadius: 6 }}>✕</button>
                      </div>
                    )
                  })}
                </div>
                ...
                <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column',
                  ...
                  <div style={{
                    ...
                    <button type="button"
                      ...
                      style={{
                        width: '100%', height: '100%', boxSizing: 'border-box' as const,
                        border: remaining <= 0 ? '1.5px solid #e5e7eb' : '1.5px solid #BFDBFE',
                        borderRadius: 9,
                        cursor: remaining <= 0 ? 'default' : 'pointer',
                        fontWeight: 700,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        userSelect: 'none' as const,
                        background: remaining <= 0 ? '#f9fafb' : '#EFF6FF',
                        color: remaining <= 0 ? '#9ca3af' : '#1565C0',
                        fontSize: 'clamp(10px, 0.9vw + 3px, 14px)',
                      }}>Yarısı</button>
                  </div>
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" onClick={() => {
                  ...
                  style={{ padding: '1vh 1.5vw', borderRadius: 8, fontSize: 'clamp(12px, 1.5vh, 13px)',
                    fontWeight: 600, cursor: 'pointer', border: '1px solid #E3E5E9', background: 'white', color: '#61656D' }}>
                  İptal
                </button>
                <button type="button" disabled={!canComplete || saving} onClick={() => void completeSale()}
                  style={{
                    padding: '1vh 1.5vw', borderRadius: 8, fontSize: 'clamp(12px, 1.5vh, 13px)',
                    fontWeight: 600, border: 'none', cursor: canComplete && !saving ? 'pointer' : 'default',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: '#D97706' }}>
              ⚠️ Tamamlanmamış Ödeme
            </div>

            <div style={{
              background: '#FEF3C7', borderRadius: 9, padding: '12px 14px',
              border: '1px solid #FCD34D', fontSize: 13,
            }}>
              <div>Sipariş No: <strong>{pavoRecoveryState.orderNo}</strong></div>
              <div style={{ marginTop: 6 }}>
                Toplam: <strong>{fmt(pavoRecoveryState.totalPrice)}</strong>
                {' · '}Ödenen: <strong>{fmt(pavoRecoveryState.paidAmount)}</strong>
                {' · '}Kalan: <strong style={{ color: '#DC2626' }}>
                  {fmt(pavoRecoveryState.remainingAmount)}
                </strong>
              </div>
            </div>

            <div style={{ fontSize: 12, color: '#6B7280' }}>
              Bu satışın bir kısmı tahsil edilmiştir.
              Kalan tutarı tahsil edebilir veya satışı iptal edebilirsiniz.
              İptal edilirse alınan ödeme iade edilir.
            </div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                onClick={() => {
                  ...
                  void (async () => {
                    if (!pavoSettings || !rec) return
                    try {
                      const seq = await window.electron.db.nextPavoSequence()
                      await pavoAbandonSuspendedSale(pavoSettings, seq, {
                        saleId:     rec.ref.saleId,
                        saleNumber: rec.ref.saleNumber,
                        orderNo:    rec.orderNo,
                      })
                    } catch (e) {
                      console.warn('[recovery] AbandonSuspendedSale hatası:', e)
                    }
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setPavoPendingSaleRef({
                    saleId:     rec.ref.saleNumber ? null : (rec.ref.saleId ?? null),
                    saleNumber: rec.ref.saleNumber ?? null,
                    orderNo:    rec.orderNo,
