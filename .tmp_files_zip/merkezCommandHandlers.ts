...
async function syncPlu(
  ...
  // Kasa bazlı ayarı oku — cashierId olmadan, id='local' satırı
  const terminalSettings = await window.electron.db.getPosSettings()
  const pluMode = terminalSettings.pluMode

  if (pluMode === 'cashier') {
    // Tüm kasiyerlerin PLU'larını çek ve her birini kasiyer bazlı kaydet
    ...
    // Terminal bazlı — cashierId göndermeden
...
/**
 * PLU'yu ekrana yansıt — giriş yapan kasiyerin gruplarını getir.
 */
async function refreshPluDisplay(
  ...
  // pluMode=cashier ise giriş yapan kasiyerin PLU'su, değilse terminal bazlı
  const terminalSettings = await window.electron.db.getPosSettings()
...
/**
 * Settings sync yardımcısı.
 * Her zaman iki kayıt yapar:
 * 1. Kasa bazlı (cashierId=null) → id='local' — pluMode, loginPolicy buradan okunur
 * 2. Kasiyer bazlı (cashierId dolu ise) → id='cashier_xxx' — PLU grid, iskonto vs.
 */
async function syncSettings(
  ...
  // 2. Kasiyer giriş yapmışsa kasiyer ayarını da yaz
  if (cashierId) {
  ...
  return {
    onSyncAll: async (mode: SyncMode = 'full') => {
      ...
      try {
        ...
        // PLU — tüm kasiyerler için ayrı ayrı
        try {
        ...
        // Settings — kasa VE kasiyer bazlı ayrı ayrı
        try {
          const workplaceId = await getWorkplaceId()
