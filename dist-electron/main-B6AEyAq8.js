...
electron.app.whenReady().then(async () => {
  ...
  electron.ipcMain.handle("db:saveProducts", async (_e2, prods) => {
    const { saveProducts } = await Promise.resolve().then(() => require("./operations-CSyFmdxq.js"));
    return saveProducts(prods);
  });
  electron.ipcMain.handle("db:getProducts", async () => {
    const { getAllProducts } = await Promise.resolve().then(() => require("./operations-CSyFmdxq.js"));
    return getAllProducts();
  });
  electron.ipcMain.handle("db:saveSale", async (_e2, sale, items) => {
    const { saveSale } = await Promise.resolve().then(() => require("./operations-CSyFmdxq.js"));
    return saveSale(sale, items);
  });
  electron.ipcMain.handle("db:getSales", async (_e2, dateFrom, dateTo) => {
    const { getSales } = await Promise.resolve().then(() => require("./operations-CSyFmdxq.js"));
  ...
  electron.ipcMain.handle("db:saveCashiers", async (_e2, cashierList) => {
    const { saveCashiers } = await Promise.resolve().then(() => require("./operations-CSyFmdxq.js"));
    return saveCashiers(cashierList);
  });
  electron.ipcMain.handle("db:verifyCashier", async (_e2, code, password) => {
    const { verifyCashier } = await Promise.resolve().then(() => require("./operations-CSyFmdxq.js"));
    return verifyCashier(code, password);
  });
  electron.ipcMain.handle("db:getCashiers", async () => {
    const { getAllCashiers } = await Promise.resolve().then(() => require("./operations-CSyFmdxq.js"));
