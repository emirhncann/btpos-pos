...
electron.app.whenReady().then(async () => {
  ...
  const { initDatabase } = await Promise.resolve().then(() => require("./index-maFpIpxk.js")).then((n) => n.index);
  ...
  electron.ipcMain.handle("app:reinitDb", async (_e, newPath) => {
    try {
      const { reinitDatabase } = await Promise.resolve().then(() => require("./index-maFpIpxk.js")).then((n) => n.index);
