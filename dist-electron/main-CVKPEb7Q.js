...
electron.app.whenReady().then(async () => {
  ...
  electron.ipcMain.handle("db:saveProducts", async (_e2, prods) => {
    const { saveProducts } = await Promise.resolve().then(() => require("./operations-bZBzpBS0.js"));
    return saveProducts(prods);
  });
  electron.ipcMain.handle("db:getProducts", async () => {
    const { getAllProducts } = await Promise.resolve().then(() => require("./operations-bZBzpBS0.js"));
    return getAllProducts();
  });
  electron.ipcMain.handle("db:saveSale", async (_e2, sale, items) => {
    const { saveSale } = await Promise.resolve().then(() => require("./operations-bZBzpBS0.js"));
    return saveSale(sale, items);
  });
  electron.ipcMain.handle("db:getSales", async (_e2, dateFrom, dateTo) => {
    const { getSales } = await Promise.resolve().then(() => require("./operations-bZBzpBS0.js"));
